...
log = function (msg, type, file, channel) {
    ...
    if(logDiscord.test(type) && discordEnabled == 1 && channel){
...
discordDeleteAllMessages = function(channel) {
    ...
    function findChannel(ch) {return ch.name === channel;}
    chJson = bot.channels.get(chanAr.find(findChannel).id)
    bot.channels.get(chJson.id).sendMessage('Gonna delete some stuff')
    chJson.fetchMessages({limit : 100}).then(function (m) {
        chJson.bulkDelete(m)
    }).catch(function (err) {
        console.log('## ' + err);
    });
...
discordDeleteMessageType = function(channel, type) {
    channel = bot.channels.find('name', config.discordRooms.rcon)
    
    channel.sendMessage('testing')
    channel.fetchMessages({limit : 2}).then(function (m) {
        console.log(m)

        // chJson.bulkDelete(messages)
    }).catch(function (err) {
        console.log('## ' + err);
    });
...
module.exports = {
  ...
  discordDeleteAllMessages: discordDeleteAllMessages,
  discordDeleteMessageType: discordDeleteMessageType
}