...
rconCommands = function(command) {
    ...
    if (co = cA.rconCommands.find(findCommand)) {
    	if (rc = co.rcon) {
            ...
            if(rc === 'giveCommand()') {giveCommand(messageArgs, command)}
...
serverGive = function(items, u, m) {
    base.log('[**GIVE** : ' + u.author.username + ' : __' + m + '__ ]', 'lcd', 'rcon.log', config.discordRooms.rcon)
...
giveCommand = function(m, u) {
    ...
    switch (m){
        case('help'):
            ...
            break;
        case('kit'):
            serverGive(itemSets.armor, u, m)
            break;
        case('c4'):
            serverGive(itemSets.c4, u, m)
            break;
        case('signal'):
            serverGive(itemSets.signal, u, m)
            break;
        case('m249'):
            serverGive(itemSets.m249, u, m)
            break;
        case('lr300'):
            serverGive(itemSets.lr300, u, m)
            break;
        default:
