...
exports.discordRcon = function (message) {
	...
	// log(line.channel.name, 'lc', logFile.info, discordRoom.bot)
}
...
rconCommands = function(command) {
    messageCommand = new RegExp(/^(.*)/).exec(command)[1]
    messageArgsRE = new RegExp(/^(.*) (.*)$/) //.exec(command)[2]
...
rconHelp = function(a) {
    ...
    for (var i in a) {
        ...
        // log(h, 'lc', logFile.info, discordRoom.bot)
    }
    return h;
}
...
serverGive = function(items, u, m, si) {
    log(si, 'lc', logFile.info, discordRoom.bot)
    log(playersConnected, 'lc', logFile.info, discordRoom.bot)
    setTimeout(function () {
    ...
    base.log('Starting the Give', 'lc', null, config.discordRooms.rcon)
    for (let i in items){
        // log('giveall ' + items[i].item + ' ' + items[i].qty, 'lc', logFile.info, discordRoom.bot)
