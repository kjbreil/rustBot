...
exports.rustBotLog = function (msg, type, file, channel) {
  ...
  if (colorMsg && RegExp(/r/).test(type)) {
    ...
    // Log to discord
  if (RegExp(/o/).test(type) && RegExp(/d/).test(type) && channel) {
    discord.discordMessage.discordSendMessage('SERVER: ' + msg, channel)
  } else if (RegExp(/d/).test(type) && channel) {
...
let colorizeMessage = (a) => {
  ...
  for (var i = 0, len = a.length; i < len; i++) {
    ...
    if (a[i].color !== undefined) {
      a[i].color = a[i].text.trim()
    }
