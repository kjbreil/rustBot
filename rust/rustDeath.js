...
exports.deathMessageIF = (line) => {
  ...
  switch (d) {
    case ('was'):
      ...
      switch (k[1]) {
        case ('killed'):
          ...
          if (p = pp.exec(line)) { pvp(line, p[1], p[2]) }					else if (o = oo.exec(line)) { gotKilled(line, o[1], o[2], o[3]) }					else log(line, 'l', logFile.chat, null)
          break
        case ('suicide'):
          var oo = new RegExp(/(.+?)\[\d+\/\d+\] \w+ (\w+) \w+ (\w+)/)
          if (o = oo.exec(line)) { gotFedUp(line, o[1], o[2], o[3]) }					else log(line, 'l', logFile.chat, null)
          break
        default:
    ...
    case ('died'):
      ...
      if (o = oo.exec(line)) { gotFedUp(line, o[1], o[2], o[3]) }			else log(line, 'l', logFile.rust, null)
...
The array is called message, below is a very basic one which passes the name of person killed and then a blank text
...
gotKilled = (line, name, how, byWhat) => {
  switch (byWhat) {
    case ('bear'):
      var message = [
{'color': 'green', 'text': name},
...
{'color': 'default', 'text': 'alive'}
      ...
      //  break;
    default:
      log('########## || ' + how + ' || ' + byWhat, 'l', null, discordRoom.chat)
  }
}
