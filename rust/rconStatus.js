...
const discordDeleteMessageType = require('./../discord/discordMessage.js').discordDeleteMessageType
...
*/
  ...
  discordDeleteMessageType(config.discordRooms.rcon, RE).then(function (z) {
    log('[*status*:***st***] ' + text, 'lcd', null, config.discordRooms.rcon)
  }).catch(function (err) {
    log(err, 'lc', logFile.info, discordRoom.bot)
  })
}

exports.returnStatus = (m, r) => {
/* This regex is fun, lots of data can be used from here.
Matching Array
1 - Server Name
2 - Server Version
...
*/
  let extractor = new RegExp(/^hostname: (.+)\nversion : (\d+) (\w+) \((.+?)\)\nmap +: (.+)\nplayers +: (\d{1,3}) \((\d{1,3}) max\) \((\d{1,3}) queued\) \((\d{1,3}) joining\)\n/)
  let e = extractor.exec(m.Message)
  let playersConnected = e[6]
  let playersQueued = e[8]
  let playersJoining = e[9]
  log(playersConnected + playersQueued + playersJoining, 'l', config.logFile.server, null)
}
